...
const crtl_POST_Login = async (req, res) => {
    ...
    if (req.user.email == config.ADMIN_MAIL) {
        ...
        console.log(`Log, de req.user en POST_Logi ${prueba}`);
        ...
        //Agrego al inicio de sesión, un token para para la autenticación de la ruta /current, que usa passport-jwt en vez de session
        // Creo el token JWT con el campo "role" basado en el rol del usuario en la base de datos, para utilizarlo luego en la creación de productos
        const tokenPayload = {
